...
VMID="101"
CT_IP="192.168.20.200"
GW="192.168.20.254"

echo "=== 1/6 - Provisioning Terraform ==="
cd terraform
terraform apply -target=proxmox_lxc.docker_admin -auto-approve
cd ..
...
echo "=== 6/6 - Configuration Ansible (Docker + Zabbix + Portainer + Dashy) ==="
cd ansible
ansible-playbook -i inventory-docker-admin.ini docker-admin.yml

echo "=== DEPLOIEMENT DOCKER-ADMIN TERMINE ! (Zabbix met qq min a demarrer) ==="
